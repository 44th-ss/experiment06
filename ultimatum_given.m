function varargout = ultimatum_given(varargin)

gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ultimatum_given_OpeningFcn, ...
                   'gui_OutputFcn',  @ultimatum_given_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ultimatum_given is made visible.
function ultimatum_given_OpeningFcn(hObject, eventdata, handles, varargin)
timePeriod = 0.01;
...
for i = 1:times
 ...
 opponent = rand;
    for j = 1:11
        if((opponent < sum(init.sample(init.datasets(i)).prob_ultimatum(1,1:j)))&&(data(number).answer(i,2) == 0))
...
data(number).answer(i,6) = game_get;
data(number).init = init;
save('ultimatum_given','data')
clear
close('ultimatum_given')
end
...
function varargout = ultimatum_given_OutputFcn(hObject, eventdata, handles) 
